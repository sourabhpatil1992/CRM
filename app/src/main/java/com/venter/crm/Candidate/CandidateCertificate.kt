...
import com.venter.crm.databinding.ActivityCandidateCertificateBinding
...
class CandidateCertificate : AppCompatActivity() {
    ...
    private fun salarySlip()
    {
        try {
            ...
            val salaryDays = EditText(this)
            val salaryDaysText = TextView(this)
            salaryDays.setHint("Salary Days")
            salaryDays.setText("0")
            salaryDays.filters = arrayOf(InputFilter.LengthFilter(2))
            salaryDays.inputType = 2
            salaryDaysText.setTextColor(resources.getColor(R.color.black))
            salaryDaysText.text ="Salary Days"
            ...
            layout.addView(newPackageText)
            layout.addView(newPackage)
            layout.addView(salaryDaysText)
            layout.addView(salaryDays)
                ...
                if (year.text.length==4 && jobPosition.text.isNotEmpty() && newPackage.text.isNotEmpty()) {
                    try {
                        ...
                        candidateViewModel.printSalarySlip(
                            ...
                            newPackage.text.toString(),lop.text.toString(),salaryByChq.isChecked,salaryDays.text.toString()
                        )
